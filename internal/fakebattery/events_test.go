...
// the type, the MicroVM it names and its payload. It then
...
func TestEveryEventTypeIsEmittedAtItsTransition(t *testing.T) {
	...
	collect := func(typ poolmgrv1.EventType) []*event {
		t.Helper()
	...
	if h := payloadOf(t, available)["host"]; h != hostA {
	...
	if p := payloadOf(t, failed); p["hook"] != string(HookCreate) || p["policy"] != poolmgrv1.HookFailurePolicy_DELETE_AND_REPLACE.String() {
...
// client recovers the state it missed.
func TestSubscribeFilterAndReplay(t *testing.T) {
	h := newHarness(t, Config{}, hostA)
	h.createPool(h.spec("one", 1, hostA))
	h.createPool(h.spec("two", 1, hostA))

	ref := h.ref("one")
	stream, err := h.client.Subscribe(h.ctx, &ref)
	if err != nil {
	...
	types := make([]poolmgrv1.EventType, 0, 3)
	for range 3 {
		e, err := stream.Recv()
		if err != nil {
			t.Fatalf("Recv: %v", err)
