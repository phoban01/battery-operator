...
// what a test gives Config.TLS on the Server side, and its client on the
// other.
...
// ServerTLS returns the Server-side material: the server certificate, and
// client certificate verification when mutual is set.
func (c *TestCerts) ServerTLS(mutual bool) *TLS {
	t := &TLS{CertFile: c.ServerCertFile, KeyFile: c.ServerKeyFile}
	if mutual {
...
// ECDSA P-256. It takes no testing.T so that code outside a test can use
// it too; callers pass t.TempDir() or clean up dir.
func WriteTestCerts(dir string, hosts ...string) (*TestCerts, error) {
	...
	caTemplate := &x509.Certificate{
		...
		Subject:               pkix.Name{CommonName: "battery-operator fake flintlockd test CA"},
		NotBefore:             notBefore,
		NotAfter:              notAfter,
		IsCA:                  true,
	...
	serverTemplate := &x509.Certificate{
		...
		Subject:      pkix.Name{CommonName: "fake flintlockd"},
		NotBefore:    notBefore,
		NotAfter:     notAfter,
	...
	clientTemplate := &x509.Certificate{
		...
		Subject:      pkix.Name{CommonName: "battery"},
		NotBefore:    notBefore,
		NotAfter:     notAfter,
