...
// expectIssuedTo checks the subject of cert: CN=node and nothing else,
// whatever the request asked for.
func expectIssuedTo(cert *x509.Certificate, node string) {
	Expect(cert.Subject.CommonName).To(Equal(node))
	Expect(cert.Subject.Organization).To(BeEmpty())
	Expect(cert.Subject.Names).To(HaveLen(1))
}

// approveAsAdmin approves the request name as the test's admin, who is not
// the Operator.
func approveAsAdmin(name string) {
	Eventually(func(g Gomega) {
		...
		g.Expect(k8sClient.SubResource("approval").Update(ctx, csr)).To(Succeed())
	}).Should(Succeed())
}

// operatorMayApprove reports whether the Operator may approve requests for
// the serving signer name.
func operatorMayApprove(g Gomega) bool {
	sar := &authorizationv1.SubjectAccessReview{Spec: authorizationv1.SubjectAccessReviewSpec{
		User: operatorUser,
		ResourceAttributes: &authorizationv1.ResourceAttributes{
			Group: certificatesv1.GroupName, Resource: "signers", Verb: "approve", Name: hostcert.ServingSigner,
		},
	}}
	g.Expect(k8sClient.Create(ctx, sar)).To(Succeed())
	return sar.Status.Allowed
}

// withholdApproval takes approve on the signer names away from the Operator
// until the end of the spec, so that the test can approve requests before
// the Operator decides on them.
func withholdApproval() {
	role := &rbacv1.ClusterRole{}
	Expect(k8sClient.Get(ctx, client.ObjectKey{Name: "manager-role"}, role)).To(Succeed())
	original := role.DeepCopy()
	for i, rule := range role.Rules {
		if slices.Contains(rule.Resources, "signers") {
			role.Rules[i].Verbs = slices.DeleteFunc(slices.Clone(rule.Verbs), func(v string) bool { return v == "approve" })
		}
	}
	Expect(k8sClient.Update(ctx, role)).To(Succeed())
	Eventually(operatorMayApprove).Should(BeFalse())
	DeferCleanup(func() {
		current := &rbacv1.ClusterRole{}
		Expect(k8sClient.Get(ctx, client.ObjectKey{Name: "manager-role"}, current)).To(Succeed())
		current.Rules = original.Rules
		Expect(k8sClient.Update(ctx, current)).To(Succeed())
		Eventually(operatorMayApprove).Should(BeTrue())
	})
}

var _ = Describe("CertificateSigningRequest Controller", Ordered, func() {
	BeforeAll(func() {
		createNode(nodeA, nodeAIP, nodeAIPv6)
		createNode(nodeB, nodeBIP)
	})

	It("should run under leader election with the leader-election Role", func() {
		lease := &coordinationv1.Lease{}
		Eventually(func(g Gomega) {
			g.Expect(k8sClient.Get(ctx, client.ObjectKey{Namespace: operatorNamespace, Name: leaderElectionID}, lease)).To(Succeed())
	...
	Context("When the Exec Agent requests its own Node's certificates", func() {
		...
		DescribeTable("should approve and sign a flintlockd serving certificate with the serving CA",
			func(ip string) {
				name := createCSR(execAgentUser, nodeA, servingRequest(nodeA, ip))
				...
				Expect(cert.IPAddresses[0].String()).To(Equal(ip))
			...
			Entry("for the Node's IPv4 address", nodeAIP),
			Entry("for the dual-stack Node's IPv6 address", nodeAIPv6),
		)
		...
		It("should approve and sign the Exec Agent's serving certificate with the serving CA", func() {
			name := createCSR(execAgentUser, nodeB, execAgentServingRequest(nodeB, nodeBIP))
			cert := waitForCertificate(name)

			Expect(servingCA.verify(cert, x509.ExtKeyUsageServerAuth)).To(Succeed())
			Expect(clientCA.verify(cert, x509.ExtKeyUsageServerAuth)).NotTo(Succeed())
			expectIssuedTo(cert, nodeB)
			Expect(cert.URIs).To(HaveLen(1))
			Expect(cert.URIs[0].String()).To(Equal("spiffe://example.test/flintlock/client/exec-agent/node-b"))
			Expect(cert.IPAddresses).To(HaveLen(1))
			Expect(cert.IPAddresses[0].String()).To(Equal(nodeBIP))
			Expect(cert.DNSNames).To(BeEmpty())
			Expect(cert.KeyUsage).To(Equal(x509.KeyUsageDigitalSignature | x509.KeyUsageKeyEncipherment))
			Expect(cert.ExtKeyUsage).To(Equal([]x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth}))
		})
	...
	Context("When a request fails a check", func() {
		...
		DescribeTable("should deny it with the check's reason, and not sign it",
			...
			Entry("an Exec Agent serving request from another ServiceAccount",
			...
			Entry("an Exec Agent serving request whose requester has no Node",
			...
			Entry("an Exec Agent serving request whose requester's Node does not exist",
			...
			Entry("an Exec Agent serving request for another Node",
				execAgentUser, nodeA, execAgentServingRequest(nodeB, nodeBIP), ReasonSubjectAltNames),
			Entry("an Exec Agent serving request with another Node's address",
			...
			Entry("a serving request with both of a dual-stack Node's addresses",
				execAgentUser, nodeA, with(servingRequest(nodeA, nodeAIP), func(r *request) {
			...
			Entry("an Exec Agent serving request with a DNS name",
			...
			Entry("an Exec Agent serving request without the Node's address",
				execAgentUser, nodeA, with(execAgentServingRequest(nodeA, nodeAIP), func(r *request) {
					r.ips = nil
				}), ReasonSubjectAltNames),
			...
			Entry("an Exec Agent serving request with flintlockd's SPIFFE ID",
				execAgentUser, nodeA, with(execAgentServingRequest(nodeA, nodeAIP), func(r *request) {
					r.uris = []string{hostcert.HostID(trustDomain, nodeA)}
				}), ReasonSubjectAltNames),
			Entry("a flintlockd serving request with the Exec Agent's SPIFFE ID",
				execAgentUser, nodeA, with(servingRequest(nodeA, nodeAIP), func(r *request) {
			...
			Entry("an Exec Agent serving request for client auth",
				execAgentUser, nodeA, with(execAgentServingRequest(nodeA, nodeAIP), func(r *request) {
					r.usages = clientUsages
				}), ReasonKeyUsages),
	...
	Context("When someone else approves a request", func() {
		BeforeEach(withholdApproval)

		It("should mark a request that fails a check Failed, and never sign it", func() {
			name := createCSR(execAgentUser, nodeA, servingRequest(nodeB, nodeBIP))
			approveAsAdmin(name)
			Eventually(func(g Gomega) {
				csr := getCSR(g, name)
				failed := condition(csr, certificatesv1.CertificateFailed)
				g.Expect(failed).NotTo(BeNil())
				g.Expect(failed.Reason).To(Equal(ReasonSubjectAltNames))
				g.Expect(failed.Message).To(ContainSubstring("Approved, but failed a check"))
			}).Should(Succeed())
			Consistently(func(g Gomega) {
				csr := getCSR(g, name)
		...
		It("should sign a request that passes every check", func() {
			name := createCSR(execAgentUser, nodeA, clientRequest(nodeA))
			approveAsAdmin(name)
			cert := waitForCertificate(name)
			Expect(clientCA.verify(cert, x509.ExtKeyUsageClientAuth)).To(Succeed())
			expectIssuedTo(cert, nodeA)
			Expect(condition(getCSR(Default, name), certificatesv1.CertificateApproved).Reason).To(Equal("AdminApproved"))
		})
	})
